  ...
  <div class="grid grid-cols-1 gap-4 sm:grid-cols-2 md:gap-6">
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          <span class="text-sm text-gray-500 dark:text-gray-400">የታራሚዎቺ ብዛት </span>
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">{{ totalPrisoner }}</h4>
        </div>
    ...
    <div class="rounded-2xl border border-gray-200 bg-white p-5 dark:border-gray-800 dark:bg-white/[0.03] md:p-6">
      ...
      <div class="flex items-end justify-between mt-5">
        <div>
          <span class="text-sm text-gray-500 dark:text-gray-400">የሰራተኞቺ ብዛት</span>
          <h4 class="mt-2 font-bold text-gray-800 text-title-sm dark:text-white/90">{{totalUser}}</h4>
        </div>
...
const totalPrisoner = ref(0);
const totalUser = ref(0);
const apiServer = computed(() => store.state.apiServer);
const percentage = ref(0); 
const percentageUser = ref(0); // Percentage for total users
const totalPrisoners = async () => {
  try {
    const response = await axios.get(apiServer.value + 'prisoner');
    if (response.data && Array.isArray(response.data.Prisioner.data)) {
      totalPrisoner.value = response.data.Prisioner.data.length;
       percentage.value = (totalPrisoner.value / 10000 * 100).toFixed(2); // Update the total prisoner count
    } else {
      console.error("Unexpected response structure:", response.data);
      totalPrisoner.value = 0; // Handle the unexpected structure
    }
  } catch (error) {
    console.error("Error fetching total prisoners:", error);
    totalPrisoner.value = 0; // Handle the error as needed
  }
}
const totalUsers = async () => {
  try {
    const response = await axios.get(apiServer.value + 'users'); // Adjust endpoint if needed
    console.log('userrr', response);

    // Check if response.data is an array
    if (response.data && Array.isArray(response.data.user)) {
      totalUser.value = response.data.user.length; // Count users
    } else {
      console.error("Unexpected response structure:", response.data);
      totalUser.value = 0; // Handle unexpected structure
    }
    percentageUser.value = (totalUser.value / 10000 * 100).toFixed(2); // Calculate percentage
  } catch (error) {
    console.error("Error fetching total users:", error);
    totalUser.value = 0; // Handle the error as needed
  }
}
// Fetch the total prisoners when the component is mounted
onMounted(() => {
  totalPrisoners();
  totalUsers();
});
