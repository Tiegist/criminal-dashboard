<template>
	<AdminLayout>
						...
						<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
							...
							<tbody>
								<tr v-for="city in cities" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
									...
									<td class="px-6 py-4">
										<a @click="editCity(city)"	class="font-medium text-blue-600 dark:text-blue-500 hover:underline">አስተካክል</a>
										|
										<a @click="confirmDelete(city)" class="font-medium text-red-600 dark:text-red-500 hover:underline">አጥፋ</a>
									</td>
								</tr>
		...
		<div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
			...
			<p class="text-gray-700 mb-6">እርግጠኛ ነዎት ለማጥፋት?  <span class="text-teal-600">{{ city.name }}</span></p>
			<div class="flex justify-end space-x-2">
				<button @click="() => isModalOpen = false" class="px-4 py-2 bg-gray-300 rounded-lg">አይ </button>
				<button @click="deleteCity()" class="px-4 py-2 bg-blue-600 bg-opacity-80 text-white rounded-lg">አዎ </button>
			</div>
			</div>
		</div>
	</AdminLayout>
