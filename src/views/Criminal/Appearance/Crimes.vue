<template>
	<AdminLayout>
						...
						<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
							...
							<tbody>
								<tr v-for="crime in crimes" :key="crime.key" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
									...
									<td class="px-6 py-4">
										<a @click="editcrime(crime)"	class="font-medium text-blue-600 dark:text-blue-500 hover:underline">Edit</a>
										|
										<a @click="confirmDelete(crime)" class="font-medium text-red-600 dark:text-red-500 hover:underline">Delete</a>
									</td>
								</tr>
		...
		<div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
			...
			<p class="text-gray-700 mb-6">Are you sure to delete <span class="text-teal-600">{{ crime.name }}</span></p>
			<div class="flex justify-end space-x-2">
				<button @click="() => isModalOpen = false" class="px-4 py-2 bg-gray-300 rounded-lg">No</button>
				<button @click="deletecrime()" class="px-4 py-2 bg-blue-600 bg-opacity-80 text-white rounded-lg">Yes</button>
			</div>
			</div>
		</div>
	</AdminLayout>
