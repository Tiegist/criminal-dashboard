<template>
	<AdminLayout>
						...
						<table class="w-full text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
							...
							<tbody>
								<tr v-for="type in types" :key="type.key" class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 border-gray-200">
									...
									<td class="px-6 py-4">
										<a @click="edittype(type)"	class="font-medium cursor-pointer text-blue-600 dark:text-blue-500 hover:underline">ያስተካክሉ</a>
										|
										<a @click="confirmDelete(type)" class="font-medium cursor-pointer text-red-600 dark:text-red-500 hover:underline">ያጥፉ</a>
									</td>
								</tr>
		...
		<div v-if="isModalOpen" class="fixed inset-0 flex items-center justify-center bg-gray-900 bg-opacity-50">
			...
			<p class="text-gray-700 mb-6">ማጥፋት ይፈልጋሉ?<span class="text-teal-600">{{ type.type }}</span></p>
			<div class="flex justify-end space-x-2">
				<button @click="() => isModalOpen = false" class="px-4 py-2 bg-gray-300 rounded-lg">አይ</button>
				<button @click="deletetype()" class="px-4 py-2 bg-blue-600 bg-opacity-80 text-white rounded-lg">አዎ</button>
			</div>
			</div>
		</div>
	</AdminLayout>
